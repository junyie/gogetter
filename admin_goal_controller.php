...
if (isset($_POST['submit']) && isset($_POST['goals_name'])){
    ...
    try{
        ...
        $insertNewGoal = insertNewGoal($conn, $goal_name, $goal_description, $goal_category);
        if ($insertNewGoal[0]=="false"){
            $conn->rollBack();     
        }else{
            $goalid = $insertNewGoal[1];
            $_SESSION['post_goal_id'] = $insertNewGoal[1];
            require_once("upload_image.php"); // pass the image to the new created goal id
            $conn->commit();
...
if (isset($_POST['update'])){
    ...
    try{
        ...
        if ($updateGoal[0]=="false"){
			...
			echo '<script>alert("Record Updated!123");</script>';
            $conn->commit();
            header("location:admin_goal_list.php");
        }
    }
...
function insertNewGoal($conn, $goal_name, $goal_description, $goal_category){

    $date_created = date("Y-m-d H:i:s"); 
    $queryIns = "INSERT INTO `common_goals`( `goals_name`, `category`, `goals_desc`,  `date_created`) VALUES (:goals_name, :goal_category, :goal_description, :date_created)"; 
